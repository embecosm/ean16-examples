...
# Basic PyTorch model to recognize 7-segement display characters, with textual
...
def log_parameters(model):
    """
    Dump out the weights and biases for the two activation layers.
    """
    layers = { 1: model.layer1, 3: model.layer3, }
    for idx, l in layers.items():
        print(f'Linear layer {idx} weights shape: {l.weight.shape}')
        print(f'Linear layer {idx} weights:')
        print(f'{l.weight.data}')
        print(f'Linear layer {idx} bias shape: {l.bias.shape}')
        print(f'Linear layer {idx} bias:')
        print(f'{l.bias.data}')
        print()

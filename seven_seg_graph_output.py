...
# Basic PyTorch model to recognize 7-segement display characters and plot some
...
class NN7Seg(nn.Module):
    ...
    We supplement this model with lists to capture training and test losses
    and test accuracy.
    """
    def __init__(self, inner_layer_size):
        ...
        # Lists to collect losses and accuracy
        self.train_losses = []
        self.test_losses = []
        self.test_accuracy = []
...
def train_model(model, learning_rate, batch_size, num_training_examples,
    ...
    for t in range(50):
        ...
        model.test_accuracy.append(acc)
...
def dump_graph(model):
    """
    Do a matlab plot of training and test losses and test accuracy.
    """
    # An X-Y plot, with a secondary access for the accuracy
    fig, ax1 = plt.subplots(figsize=(11.69,8.27), dpi=300)
    ax2 = ax1.twinx()
    # We'll need to be explict about the colors with two axes
    prop_cycle = plt.rcParams['axes.prop_cycle']
    colors = prop_cycle.by_key()['color']
    # Plot the loss graphs on one axis, accuracy on the other
    ax1.plot(model.train_losses, lw=2, color=colors[0], label='training losses')
    ax1.plot(model.test_losses, lw=2, color=colors[1], label='test losses')
    ax2.plot(model.test_accuracy, lw=2, color=colors[2], label='test accuracy')
    # Label things nicely.
    ...
    plt.title(f'7 segment display losses')
    # Dump out the graph
    fn = 'seven_seg_graph'
    fmt = 'svg'
    plt.savefig(f'{fn}.{fmt}', format = fmt)
    plt.close(fig)
    print(f'Losses plotted in {fn}.{fmt}')
